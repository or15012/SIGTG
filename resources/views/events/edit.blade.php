...
@section('content')
    ...
    <div class="container">
        <div class="contenedor">
            ...
            <a href="{{ route('events.index', ['project' => $project->id]) }}" class="btn btn-danger regresar-button">
        ...
        <form action="{{ route('events.update', ['project' => $project->id, 'events' => $events->id]) }}" method="POST" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="mb-3">
                ...
                <input class="form-control" type="text" id="name" name="name" value="{{ $events->name }}">
            </div>

            <div class="mb-3">
                <label for="description" class="form-label">Descripción:</label>
                <input class="form-control" type="text" id="description" name="description" value="{{ $events->description }}">
            </div>

            <div class="mb-3">
                <label for="place" class="form-label">Lugar:</label>
                <input class="form-control" type="text" id="place" name="place" value="{{ $events->place}}">
            </div>

            <div class="mb-3">
                <label for="date" class="form-label">Fecha:</label>
                <input class="form-control" type="datetime-local" id="date" name="date"
                    value="{{ \Carbon\Carbon::parse($events->date)->format('Y-m-d\TH:i') }}"
