...
@section('content')
    ...
    <div class="container">
        ...
        <form action="{{ route('extensions.update', $extension->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <option value=""> Seleccione un proyecto </option>
                    @foreach ($projects as $project)
                        <option value="{{$project->id}}" @if($project->id == $extension->project_id) selected @endif> {{$project->name}}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
                ...
                <option value=""> Seleccione un tipo de extensión</option>
                    @foreach ($type_extensions as $type_extension)
                        <option value="{{$type_extension->id}}" @if($type_extension->id == $extension->type_extension_id) selected @endif> {{$type_extension->name}}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                <label for="extension_request_path" class="form-label">Solicitud de prórroga <a href="{{route('download', ['file'=>$extension->extension_request_path])}}">Descargar</a></label>
                <input type="file" class="form-control" id="extension_request_path" name="extension_request_path">
            </div>
            <div class="mb-3">
                <label for="schedule_activities_path" class="form-label">Cronograma de actividades <a href="{{route('download', ['file'=>$extension->schedule_activities_path])}}">Descargar</a></label>
                <input type="file" class="form-control" id="schedule_activities_path" name="schedule_activities_path">
            </div>
            <div class="mb-3">
                <label for="approval_letter_path" class="form-label">Carta de aprobación de asesor <a href="{{route('download', ['file'=>$extension->approval_letter_path])}}">Descargar</a></label>
