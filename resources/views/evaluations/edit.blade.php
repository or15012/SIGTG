    ...
    <div class="container">
        ...
        <form action="{{ route('subareas.criterias.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="percentage" name="percentage" value="{{ $evaluation->percentage }}"
                    required>
            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control select2" id="subareas" name="subareas[]" multiple>
                                @foreach ($subareas as $subarea)
                                    <option value="{{ $subarea->id }}" {{ in_array($subarea->id, $selectedSubareas) ? 'selected' : '' }}>{{ $subarea->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    @case(5)
                        <div class="mb-3">
                            ...
                            <select class="form-control" id="type" name="type" required>
                                ...
                                <option value="1" @if($evaluation->type == 1) selected @endif>Con entrega de documentos</option>
                                <option value="0" @if($evaluation->type == 0) selected @endif>Sin entrega de documentos</option>
                            </select>
                        </div>
                    @break
