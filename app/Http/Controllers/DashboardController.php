    ...
    public function index(Request $request)
    {
        ...
        $datos3 = DB::table('groups AS g')
            ->select(
                ...
                DB::raw('COUNT(DISTINCT g.id) as cantidad')
            )
            ->join('protocols AS p', 'g.protocol_id', '=', 'p.id')
            ->join('user_group as ug', 'g.id', 'ug.group_id')
            ->join('users as u', 'ug.user_id', 'u.id')
            ->join('schools as sch', 'sch.id', 'u.school_id')
            ->where('g.cycle_id', '=', $actualCycle->id)
            ->groupBy('sch.id', 'sch.name', 'p.id', 'p.name');

        // Filtrar por la escuela si no es una selección global
        if (session('school')['id'] != -1) {
            $datos3->where('sch.id', session('school')['id']);
        }
        ...
        $datos4 = DB::table('extensions as ext')
            ->select(
                ...
                'p.id as protocol_id',
                'p.name as protocol_name',
                DB::raw('COUNT(DISTINCT ext.id) as cantidad')
            )
            ...
            ->where('g.cycle_id', '=', $actualCycle->id);
        if (session('school')['id'] != -1) {
            $datos4->where('sch.id', session('school')['id']);
        }
        $datos4 = $datos4->get();
